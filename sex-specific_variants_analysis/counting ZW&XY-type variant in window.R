...
snp_data_xy <- read.table("XY_pos.txt", col.names = c('Chromosome', 'Position')) %>%
  mutate(Variant = "XY-type variants")  # 雄性專屬

snp_data_zw <- read.table("ZW_pos.txt", col.names = c('Chromosome', 'Position')) %>%
  mutate(Variant = "ZW-type variants")  # 雌性專屬
...
density_zw <- compute_sliding_density(snp_data_zw, chromosomes, window_size, step_size, "ZW-type variants")
density_xy <- compute_sliding_density(snp_data_xy, chromosomes, window_size, step_size, "XY-type variants")

# Combine and assign chromosome offset positions
plot_data <- bind_rows(density_zw, density_xy)
...
p <- ggplot(plot_data, aes(x = x, y = Count, color = Variant, group = interaction(Chromosome, Variant))) +
  geom_line(alpha = 0.7) +
  scale_color_manual(values = c("ZW-type variants" = "red", "XY-type variants" = "blue")) +
  scale_x_continuous(breaks = tick_positions, labels = tick_labels) +
  labs(
    title = "ZW-type vs XY-type Variant Density",
    x = "Position (Mbp)",
...
# Save the plot
ggsave("ZW_vs_XY_sliding_window_density_fixed.png", plot = p, width = 17, height = 5, dpi = 500)