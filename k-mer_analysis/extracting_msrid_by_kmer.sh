...
KMER_FILE="/work/u8356556/AG_kmer/m_specific_kmer_10.txt"
...
for SAMPLE in "${SAMPLES[@]}"; do
    ...
    R1_ID="$OUTPATH/${SAMPLE}_R1_msr_ids.txt"
    R2_ID="$OUTPATH/${SAMPLE}_R2_msr_ids.txt"
...
#################### 合併R1和R2的結果去除重複 ####################
COMBINED_ID="$OUTPATH/${SAMPLE}_combined_msr_ids.txt"
cat "$R1_ID" "$R2_ID" | sort | uniq > "$COMBINED_ID"
